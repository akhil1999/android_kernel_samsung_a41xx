...
irqreturn_t smcdsd_abd_handler(int irq, void *dev_id)
{
	...
	struct abd_sub_info *pin_handler = NULL;
...
{
	struct mipi_dsi_lcd_common *container = get_lcd_common(0);
	struct abd_sub_info *sub_printer = NULL, *tmp = NULL;
	...
	list_for_each_entry_safe(sub_printer, tmp, &abd->printer_list, node) {
		WARN(sub_printer->show == show && sub_printer->dev_id == data,
			"%s: already registered printer\n", __func__);

		sub_printer = kzalloc(sizeof(struct abd_sub_info), GFP_KERNEL);
		if (!sub_printer) {
			dbg_info("%s: printer kzalloc fail\n", __func__);
			break;
		}
		sub_printer->show = show;
		sub_printer->dev_id = data;
		list_add_tail(&sub_printer->node, &abd->printer_list);
...
int smcdsd_abd_pin_register_handler(struct abd_protect *abd, int irq, irq_handler_t handler, void *dev_id)
{
	struct mipi_dsi_lcd_common *container = get_lcd_common(0);
	struct abd_pin_info *pin = NULL;
	struct abd_sub_info *pin_handler = NULL, *tmp = NULL;
	unsigned int i = 0;
	...
	for (i = 0; i < ABD_PIN_MAX; i++) {
		...
		if (pin && irq == pin->irq) {
			...
			pin_handler = kzalloc(sizeof(struct abd_sub_info), GFP_KERNEL);
			if (!pin_handler) {
...
static int smcdsd_abd_show(struct seq_file *m, void *unused)
{
	...
	unsigned int i = 0;
	struct abd_sub_info *sub_printer = NULL;
	...
	list_for_each_entry(sub_printer, &abd->printer_list, node) {
		if (sub_printer && sub_printer->show)
			sub_printer->show(m, sub_printer->dev_id);
	}
					...
					size_t count, loff_t *ppos)
{
	...
	char ibuf[80] = {0, };
	int ret = 0;
	size_t sizeof_ibuf = sizeof(ibuf);
...
static void _smcdsd_abd_blank(struct abd_protect *abd)
{
...
static irqreturn_t smcdsd_abd_blank_handler(int irq, void *dev_id)
{
	struct abd_protect *abd = (struct abd_protect *)dev_id;
...
static irqreturn_t smcdsd_abd_con_handler(int irq, void *dev_id)
{
	struct abd_protect *abd = (struct abd_protect *)dev_id;
...
int smcdsd_abd_pin_register_blank_handler(struct abd_protect *abd, int irq)
{
	if (!abd->init_done)
		return -EINVAL;

	return smcdsd_abd_pin_register_handler(abd, irq, smcdsd_abd_blank_handler, abd);
}
...
static int smcdsd_abd_con_pin_register_hanlder(struct abd_protect *abd)
{
	...
	if (pin.level != pin.active_level) {
		...
		smcdsd_abd_pin_register_handler(abd, pin.irq, smcdsd_abd_blank_handler, abd);
	}
exit:
	return ret;
}
