...
static int get_sensor_scanning_info(void)
{
	int ret = 0;
	char *buffer = NULL;
	unsigned int buffer_length = 0;

	ret = shub_send_command_wait(CMD_GETVALUE, TYPE_MCU, SENSOR_SCAN_RESULT, 1000, NULL, 0, &buffer, &buffer_length,
				     true);
	if (ret < 0) {
...
int parsing_sensor_spec(char *dataframe, int *index)
{
	...
	*index += (size + 1);
	...
	if (!sensor_manager->sensor_spec) {
		sensor_manager->sensor_spec = kcalloc(total_count, sizeof(struct sensor_spec_t), GFP_KERNEL);
		received_count = 0;
	}
